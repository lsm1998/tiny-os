...
PID_FILE="${PID_FILE:-$PROJECT_DIR/.vscode/qemu-debug.pid}"
QEMU_LOG="${QEMU_LOG:-$PROJECT_DIR/.vscode/qemu-debug.log}"
COMMAND="${1:-prepare}"
...
ensure_port_free() {
    if lsof -nPiTCP:"$GDB_PORT" -sTCP:LISTEN >/dev/null 2>&1; then
        ...
        echo "hint: run 'bash script/vscode-debug-osx.sh stop' or choose another GDB_PORT"
        exit 1
    fi
}
...
prepare() {
    ...
    echo "[4/4] QEMU gdb stub is ready on 127.0.0.1:$GDB_PORT"
...
stop() {
    stop_managed_qemu
    stop_qemu_on_port_if_needed
    echo "QEMU debug session stopped"
}
